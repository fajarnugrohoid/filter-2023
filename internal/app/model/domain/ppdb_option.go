...
type PpdbOption struct {
	...
	SchoolId    primitive.ObjectID `bson:"school_id,omitempty"`
	Filtered    int
	UpdateQuota bool
	NeedQuota   int
	//AddQuota            int
...
func GetSchoolIdByOptChoice(element primitive.ObjectID, data []*PpdbOption) primitive.ObjectID {
	for _, v := range data {
		if element.String() == v.Id.String() {
			return v.SchoolId
		}
	}
	return primitive.ObjectID{} //not found.
}
